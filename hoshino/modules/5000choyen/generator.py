...
upper_font_path = path.join(path.dirname(__file__), '思源黑体CN.otf')
downer_font_path = path.join(path.dirname(__file__), '思源宋体CN.otf')

def change_font(upper, downer):
    global upper_font_path, downer_font_path
    upper_font_path = path.join(path.dirname(__file__), upper)
    downer_font_path = path.join(path.dirname(__file__), downer)
...
def getTextWidth(text, font, width=100, height=300, recursive=False):
    print(text)
    step = 100
...
def genBaseImage(width=1500, height=500):
    ...
    downerSilverArray = createLinearGradient({
        ...
        1: rgb(50, 50, 50)
    }, width=width, height=height, hsize = 0.75)
    ...
    strokeRedArray = createLinearGradient({
        ...
        0.51: rgb(240, 0, 0),
        1: rgb(5, 0, 0)
    }, width=width, height=height, size=0.82, hsize = 0.75)
    ...
    silver2Array = createLinearGradient({
        ...
        1: rgb(255, 255, 255)
    }, width=width, height=height, size=0.85, hsize = 0.75)
...
def genImage(word_a="5000兆円", word_b="欲しい!", default_width=1000, height=300,
    ...
    print(upper_font_path, downer_font_path)
    # Prepare Width
    ...
    # Draw text - Upper
    img_upper = Image.new("RGBA", (upper_width, _round(height/2 + 50)), alpha)
    ...
    # Draw text - Downer
    img_downer = Image.new("RGBA", (downer_width+leftmargin, _round(height/2 + 50)), alpha)
...
#genImage(word_a="🐍🐓⑧断了", word_b="透你🐎🍺！").save("./temp.png")
