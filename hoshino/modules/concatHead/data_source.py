...
async def detect_face(imgb64: str, max_head : int) -> dict:
    global token
...
def concat(img: Image.Image, head: KyaruHead, face_dat, user_scale : float, rotate_deg : int, x_offset : int, y_offset : int) -> Image.Image:
    lm = face_dat['landmark72']
    scale = distance_between_point(lm[0], lm[12]) * user_scale / head.face_width / 0.97  * user_scale # 设置缩放， 0.97为系数，无实际意义
    w, h = head.img.size
    head_img = head.img.resize((int(scale * w), int(scale * h)), Image.BILINEAR)
    head_img = head_img.rotate(-face_dat['rotation'] - head.angle + rotate_deg, center=(head.X * scale, head.Y * scale), resample=Image.BILINEAR)
    img.paste(head_img, (int(lm[6]['x'] - head.X * scale + x_offset), int(lm[6]['y'] - head.Y * scale + y_offset)), mask=head_img.split()[3])
    return img
